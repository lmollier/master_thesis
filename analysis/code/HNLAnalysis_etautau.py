...
class HNLAnalysis_etautau(processor.ProcessorABC):
    def __init__(self):
    ...
    def get_var_axis_pairs():
        ...
        ('pt_sum_etau', pt_axis),
        ('pt_sum_etau2', pt_axis),
        ('pt_sum_etau2tau', pt_axis),
        ('pt_sum_tau2tauMET', pt_axis),
        ('pt_sum_etauMET', pt_axis),
        ('pt_sum_etau2MET', pt_axis),
        ('pt_sum_etau2tauMET', pt_axis),
        ('comb_mass_tau2tau', mc_axis),
        ('comb_mass_etau', mc_axis),
        ('comb_mass_etau2', mc_axis),
    ...
    def process(self, events):
        ...
        events['SelTau'] = events.Tau[(events.Tau.pt > 20.) & (abs(events.Tau.eta) < 2.3)& (events.Tau.idDeepTau2017v2p1VSmu >=2) & (events.Tau.idDeepTau2017v2p1VSe >= 8) & (events.Tau.idDeepTau2017v2p1VSjet >=64)]
    ...
    def save_sum_pt(self, events, out, ds, mode, tag=''):
        ...
        MET_pt = events.DeepMETResolutionTune.pt
        ...
        out[f'pt_sum_etau2{tag}'].fill(ds=ds, pt=ak.flatten((lead_tau_2.pt + lead_electron.pt), axis=None), weight=events.weight)
        out[f'pt_sum_etau2tau{tag}'].fill(ds=ds, pt=ak.flatten((lead_tau_2.pt + lead_electron.pt  + lead_tau.pt ), axis=None), weight=events.weight)
        ...
        out[f'pt_sum_etau2MET{tag}'].fill(ds=ds, pt=ak.flatten((lead_tau_2.pt + lead_electron.pt+ MET_pt), axis=None), weight=events.weight)
        out[f'pt_sum_etau2tauMET{tag}'].fill(ds=ds, pt=ak.flatten((lead_tau_2.pt + lead_electron.pt  + lead_tau.pt+ MET_pt ), axis=None), weight=events.weight)
    ...
    def save_combined_mass(self, events, out, ds, mode, tag=''):
        ...
        out[f'comb_mass_etau2{tag}'].fill(ds=ds, mc=ak.flatten((lead_tau_2 + lead_electron).mass, axis=None), weight=events.weight)
    ...
    def save_combined_var(self, events, out, ds, mode, tag=''):
        ...
        out[f'dr_etau2{tag}'].fill(ds=ds, dr=ak.flatten(delta_r(lead_electron,lead_tau_2), axis=None), weight=events.weight)
        ...
        out[f'dphi_etau2{tag}'].fill(ds=ds, phi=ak.flatten(delta_phi(lead_electron,lead_tau_2), axis=None), weight=events.weight)
        ...
        out[f'deta_etau2{tag}'].fill(ds=ds, eta=ak.flatten(delta_eta(lead_electron,lead_tau_2), axis=None), weight=events.weight)
    ...
    def save_transverse_mass(self, events, out, ds, mode, tag=''):
        ...
        MET_phi = events.DeepMETResolutionTune.phi
        MET_pt = events.DeepMETResolutionTune.pt
        ...
        cos_etau2 = np.cos(abs(electron_phi - tau2_phi))
        ...
        transverse_mass_etau2 = np.sqrt(electron_mass_sq + tau2_mass_sq + 2*(electron_energy*tau2_energy - electron_pt*tau2_pt*cos_etau2))
        ...
        MT_total = np.sqrt(transverse_mass_etau2**2 +transverse_mass_etau**2 +transverse_mass_tau2tau**2 +transverse_mass_eMET**2 +transverse_mass_tau2MET**2 +transverse_mass_tauMET**2  )        
        ...
        out[f'transverse_mass_etau2{tag}'].fill(ds=ds, mt=ak.flatten(transverse_mass_etau2, axis=None), weight=events.weight)
    ...
    def save_MET(self, events, out, ds, mode, tag=''):
        print("save_MET " + ds)
        out[f'pt_MET{tag}'].fill(ds=ds, pt=ak.flatten(events.DeepMETResolutionTune.pt, axis=None), weight=events.weight)
        out[f'phi_MET{tag}'].fill(ds=ds, phi=ak.flatten(events.DeepMETResolutionTune.phi, axis=None), weight=events.weight)
    ...
    def analyse_etautau(self, events, out, ds, mode):
        ...
        out['sumw_etau2tau'][ds] += ak.sum(events.genWeight)
        print("**********************")
