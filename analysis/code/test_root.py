...
#compute the error
(h_CR_loose,bins_CR_loose) = np.histogram(CR_loose_kin, bins = bins_pt_tau)
(h_CR_medium,bins_CR_medium) = np.histogram(CR_medium_kin, bins = bins_pt_tau)
...
#compute the ratio 
(h_CR_loose,bins_CR_loose) = np.histogram(CR_loose_kin, bins = bins_pt_tau, weights = CR_loose.genWeight * scale)
(h_CR_medium,bins_CR_medium) = np.histogram(CR_medium_kin, bins = bins_pt_tau, weights = CR_medium.genWeight * scale)
h_ratio_CR_loose_med = np.divide(h_CR_loose,h_CR_medium)
h_ratio_CR_loose_med = np.nan_to_num(h_ratio_CR_loose_med, nan=0, posinf=0, neginf=0)
...
#compute the error
h = np.histogram2d(SR_loose_kin,SR_var1_loose_kin, bins = [bins_pt_muon, bins_pt_tau])
h = h[0]
h_sqrt = np.sqrt(h) 
...
h = np.histogram2d(SR_loose_kin,SR_var1_loose_kin, bins = [bins_pt_muon, bins_pt_tau], weights=SR_loose.genWeight* scale)
h = h[0]
for i in range(0, len(h)):
    for j in range(0, len(h[i])):
...
#compute the error
(h_SR_medium,bins_SR_medium) = np.histogram(SR_medium_kin, bins = bins_pt_muon)
...
(h_SR_medium,bins_SR_medium) = np.histogram(SR_medium_kin, bins = bins_pt_muon, weights = SR_medium.genWeight* scale)
...
#compute the error
h = np.histogram2d(SR_loose_kin,SR_var1_loose_kin, bins = [bins_pt_var_now, bins_pt_tau])
h = h[0]
h_sqrt = np.sqrt(h) 
h_error_temp = (h_sqrt/h)**2
h_error_temp = np.nan_to_num(h_error_temp, nan=0, posinf=0, neginf=0)
...
#compute the histogram scaled
h = np.histogram2d(SR_loose_kin,SR_var1_loose_kin, bins = [bins_pt_var_now, bins_pt_tau], weights=SR_loose.genWeight* scale)
h = h[0]
for i in range(0, len(h)):
    for j in range(0, len(h[i])):
        h[i][j] = h[i][j] / h_ratio_CR_loose_med[j]
...
#compute the error
(h_SR_medium,bins_SR_medium) = np.histogram(SR_medium_kin, bins = bins_pt_var_now)
ratio_SR_kin = np.divide(h_SR_medium, h_SR_loose_to_med_for_error)
ratio_SR_kin = np.nan_to_num(ratio_SR_kin, nan=0, posinf=0, neginf=0)
error_SR_medium = np.sqrt(h_SR_medium)
temp1 = (error_SR_loose_to_med/h_SR_loose_to_med_for_error)**2
...
#compute the histogram with scaling
(h_SR_medium,bins_SR_medium) = np.histogram(SR_medium_kin, bins = bins_pt_var_now, weights = SR_medium.genWeight* scale)
ratio_SR_kin = np.divide(h_SR_medium, h_SR_loose_to_med)
ratio_SR_kin = np.nan_to_num(ratio_SR_kin, nan=0, posinf=0, neginf=0)
...
#compute the error
h = np.histogram2d(SR_loose_kin,SR_var1_loose_kin, bins = [bins_var_now, bins_pt_tau])
h = h[0]
h_sqrt = np.sqrt(h) 
h_error_temp = (h_sqrt/h)**2
h_error_temp = np.nan_to_num(h_error_temp, nan=0, posinf=0, neginf=0)
...
#compute the histogram scaled
h = np.histogram2d(SR_loose_kin,SR_var1_loose_kin, bins = [bins_var_now, bins_pt_tau], weights=SR_loose.genWeight* scale)
h = h[0]
for i in range(0, len(h)):
    for j in range(0, len(h[i])):
        h[i][j] = h[i][j] / h_ratio_CR_loose_med[j]
...
#compute the error
(h_SR_medium,bins_SR_medium) = np.histogram(SR_medium_kin, bins = bins_var_now)
ratio_SR_kin = np.divide(h_SR_medium, h_SR_loose_to_med_for_error)
ratio_SR_kin = np.nan_to_num(ratio_SR_kin, nan=0, posinf=0, neginf=0)
error_SR_medium = np.sqrt(h_SR_medium)
temp1 = (error_SR_loose_to_med/h_SR_loose_to_med_for_error)**2
...
#compute the histogram with scaling
(h_SR_medium,bins_SR_medium) = np.histogram(SR_medium_kin, bins = bins_var_now, weights = SR_medium.genWeight* scale)
ratio_SR_kin = np.divide(h_SR_medium, h_SR_loose_to_med)
ratio_SR_kin = np.nan_to_num(ratio_SR_kin, nan=0, posinf=0, neginf=0)
...
ax.set_ylim([0, 2])
ax.set_xlim([0,3.2])
plt.savefig("fake_rates_ttbar.pdf")
plt.show()
